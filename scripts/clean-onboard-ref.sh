...
Environment overrides:
  ...
  PC_DATA=/tmp/data           ZeroInc data dir to use
...
export PC_TEST_ROOT="${PC_TEST_ROOT:-$(mktemp -d /tmp/zeroinc-clean-ref.XXXXXX)}"
export PC_HOME="${PC_HOME:-$PC_TEST_ROOT/home}"
export PC_CACHE="${PC_CACHE:-$PC_TEST_ROOT/npm-cache}"
export PC_DATA="${PC_DATA:-$PC_TEST_ROOT/zeroinc-data}"
...
env \
  ...
  pnpm zeroinc onboard --yes --data-dir "$PC_DATA"
