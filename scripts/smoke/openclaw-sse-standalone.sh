...
OPENCLAW_USER="${OPENCLAW_USER:-zeroinc-smoke}"
...
Run your ZeroInc heartbeat procedure now.
EOF
...
PAYLOAD="$(jq -nc \
  ...
  '{
    ...
    metadata: {
      ...
      zeroinc_session_key: ("zeroinc:run:" + $runId)
    }
  }')"
...
args=(
  ...
  -H "x-openclaw-session-key: zeroinc:run:${PAPERCLIP_RUN_ID}"
  -D "$headers_file"
  -o "$body_file"
  --data "$PAYLOAD"
  "$OPENCLAW_URL"
