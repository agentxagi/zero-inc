...
COMPOSE_OVERRIDE="${OPENCLAW_DOCKER_DIR}/.zeroinc-openclaw.override.yml"
...
zeroinc_base_url="$(detect_zeroinc_base_url || true)"
...
ZeroInc URL for OpenClaw container:
EOF
  if [[ -n "$zeroinc_base_url" ]]; then
    cat <<EOF
  $zeroinc_base_url
...
EOF
  ...
  If ZeroInc rejects the host, run on host machine:
    pnpm zeroinc allowed-hostname host.docker.internal
  Then restart ZeroInc and re-run this script.
EOF
  fi
  cat <<EOF
