...
OPENCLAW_CONFIG_DIR="${OPENCLAW_CONFIG_DIR:-${OPENCLAW_TMP_DIR}/openclaw-zeroinc-smoke}"
...
create_and_approve_gateway_join() {
  ...
  join_payload="$(jq -nc \
    ...
    --arg zeroincApiUrl "$PAPERCLIP_API_URL_FOR_OPENCLAW" \
...
persist_claimed_key_artifacts() {
  ...
  local skill_dir="${OPENCLAW_CONFIG_DIR%/}/skills/zeroinc"
  local claimed_file="${workspace_dir}/zeroinc-claimed-api-key.json"
  local claimed_raw_file="${workspace_dir}/zeroinc-claimed-api-key.raw.json"
  ...
  if [[ -n "$container" ]]; then
    ...
    docker cp "$claimed_file" "${container}:/home/node/.openclaw/workspace/zeroinc-claimed-api-key.json" >/dev/null 2>&1 || true
    docker exec "$container" sh -lc "chmod 644 /home/node/.openclaw/workspace/zeroinc-claimed-api-key.json" >/dev/null 2>&1 || true
  fi

  if [[ "$AUTO_INSTALL_SKILL" == "1" ]]; then
    api_request "GET" "/skills/zeroinc"
    assert_status "200"
    {
...
inject_agent_api_key_payload_template() {
  ...
  local base_message
  base_message="Set PAPERCLIP_API_KEY=${AGENT_API_KEY} in your run context before running ZeroInc heartbeat steps."
...
main() {
  ...
  wait_http_ready "${PAPERCLIP_API_URL%/}/api/health" 15 || fail "ZeroInc API health endpoint not reachable"
  api_request "GET" "/health"
  assert_status "200"
  log "zeroinc health deploymentMode=$(jq -r '.deploymentMode // "unknown"' <<<"$RESPONSE_BODY") exposure=$(jq -r '.deploymentExposure // "unknown"' <<<"$RESPONSE_BODY")"
