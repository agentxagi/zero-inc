...
IMAGE_NAME="${IMAGE_NAME:-zeroinc-onboard-smoke}"
...
SMOKE_ADMIN_EMAIL="${SMOKE_ADMIN_EMAIL:-smoke-admin@zeroinc.local}"
SMOKE_ADMIN_PASSWORD="${SMOKE_ADMIN_PASSWORD:-zeroinc-smoke-password}"
...
generate_bootstrap_invite_url() {
  ...
  if bootstrap_output="$(
    docker exec \
      ...
      'timeout 20s npx --yes "zeroinc@${PAPERCLIPAI_VERSION}" auth bootstrap-ceo --data-dir "$PAPERCLIP_HOME" --base-url "$PAPERCLIP_PUBLIC_URL"' \
      2>&1
  )"; then
    bootstrap_status=0
  else
...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/zeroinc-onboard-smoke.XXXXXX")"
