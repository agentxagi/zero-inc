...
import {
  heartbeatService,
  productCouncilAutomationService,
...
export async function startServer(): Promise<StartedServer> {
  ...
  const LOCAL_BOARD_USER_EMAIL = "local@zeroinc.local";
  ...
  if (config.databaseUrl) {
    ...
    if (runningPid) {
      ...
      const configuredAdminConnectionString = `postgres://zeroinc:zeroinc@127.0.0.1:${configuredPort}/postgres`;
      try {
        ...
        await ensurePostgresDatabase(configuredAdminConnectionString, "zeroinc");
        logger.warn(
    ...
    const embeddedAdminConnectionString = `postgres://zeroinc:zeroinc@127.0.0.1:${port}/postgres`;
    const dbStatus = await ensurePostgresDatabase(embeddedAdminConnectionString, "zeroinc");
    if (dbStatus === "created") {
      logger.info("Created embedded PostgreSQL database: zeroinc");
    }
  
    const embeddedConnectionString = `postgres://zeroinc:zeroinc@127.0.0.1:${port}/zeroinc`;
  ...
  if (config.heartbeatSchedulerEnabled) {
    ...
    const councilAutomation = productCouncilAutomationService(db as any);
  
    // Reap orphaned running runs at startup while in-memory execution state is empty,
    // then resume any persisted queued runs that were waiting on the previous process.
    void heartbeat
      .reapOrphanedRuns()
      .then(() => heartbeat.cleanupStaleExecutionLocks())
    ...
    setInterval(() => {
      void heartbeat
        .tickTimers(new Date())
        .then((result) => {
          if (result.enqueued > 0 || result.skippedByPreventiveQuota > 0) {
            logger.info({ ...result }, "heartbeat timer tick summary");
          }
        })
        .catch((err) => {
      ...
      void councilAutomation
        .tick(new Date())
        .then((result) => {
          if (result.generatedIssues > 0 || result.errors > 0) {
            logger.info({ ...result }, "product council automation tick summary");
          }
        })
        .catch((err) => {
          logger.error({ err }, "product council automation tick failed");
        });
...
if (isMainModule(import.meta.url)) {
  void startServer().catch((err) => {
    logger.error({ err }, "ZeroInc server failed to start");
    process.exit(1);
  });
}
