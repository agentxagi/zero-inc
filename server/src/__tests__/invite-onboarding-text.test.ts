...
describe("buildInviteOnboardingTextDocument", () => {
  it("renders a plain-text onboarding doc with expected endpoint references", () => {
    ...
    expect(text).toContain("ZeroInc OpenClaw Gateway Onboarding");
    ...
    expect(text).toContain("Suggested ZeroInc base URLs to try");
    expect(text).toContain("http://localhost:3100");
    expect(text).toContain("host.docker.internal");
    expect(text).toContain("zeroincApiUrl");
    ...
    expect(text).toContain("set the first reachable candidate as agentDefaultsPayload.zeroincApiUrl");
    expect(text).toContain("~/.openclaw/workspace/zeroinc-claimed-api-key.json");
