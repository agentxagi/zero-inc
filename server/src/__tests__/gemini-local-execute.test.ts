...
import { execute } from "@zeroinc/adapter-gemini-local/server";
...
describe("gemini execute", () => {
  it("passes prompt via --prompt and injects paperclip env vars", async () => {
    ...
    try {
      ...
      expect(promptArg).toContain("Follow the zeroinc heartbeat.");
      expect(promptArg).toContain("ZeroInc runtime note:");
      ...
      expect(invocationPrompt).toContain("ZeroInc API access note:");
  ...
  it("always passes --approval-mode yolo", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "zeroinc-gemini-yolo-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "gemini");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
