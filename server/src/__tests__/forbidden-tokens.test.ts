...
describe("forbidden token check", () => {
  it("derives username tokens without relying on whoami", () => {
    const tokens = resolveDynamicForbiddenTokens(
      { USER: "zeroinc", LOGNAME: "zeroinc", USERNAME: "pc" },
      {
        userInfo: () => ({ username: "zeroinc" }),
      },
    );

    expect(tokens).toEqual(["zeroinc", "pc"]);
  });
  ...
  it("merges dynamic and file-based forbidden tokens", async () => {
    ...
    try {
      const tokens = resolveForbiddenTokens(tokensFile, { USER: "zeroinc" }, {
        userInfo: () => ({ username: "zeroinc" }),
      });

      expect(tokens).toEqual(["zeroinc", "custom-token"]);
    } finally {
