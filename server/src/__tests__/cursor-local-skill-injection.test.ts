...
import { ensureCursorSkillsInjected } from "@zeroinc/adapter-cursor-local/server";
...
describe("cursor local adapter skill injection", () => {
  ...
  it("links missing ZeroInc skills into Cursor skills home", async () => {
    const skillsDir = await makeTempDir("zeroinc-cursor-skills-src-");
    const skillsHome = await makeTempDir("zeroinc-cursor-skills-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);

    await createSkillDir(skillsDir, "zeroinc");
    await createSkillDir(skillsDir, "zeroinc-create-agent");
    ...
    const injectedA = path.join(skillsHome, "zeroinc");
    const injectedB = path.join(skillsHome, "zeroinc-create-agent");
    expect((await fs.lstat(injectedA)).isSymbolicLink()).toBe(true);
    expect((await fs.lstat(injectedB)).isSymbolicLink()).toBe(true);
    expect(await fs.realpath(injectedA)).toBe(await fs.realpath(path.join(skillsDir, "zeroinc")));
    expect(await fs.realpath(injectedB)).toBe(
      await fs.realpath(path.join(skillsDir, "zeroinc-create-agent")),
    );
    expect(logs.some((line) => line.includes('Injected Cursor skill "zeroinc"'))).toBe(true);
    expect(logs.some((line) => line.includes('Injected Cursor skill "zeroinc-create-agent"'))).toBe(true);
  });

  it("preserves existing targets and only links missing skills", async () => {
    const skillsDir = await makeTempDir("zeroinc-cursor-preserve-src-");
    const skillsHome = await makeTempDir("zeroinc-cursor-preserve-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);
    ...
    const existingTarget = path.join(skillsHome, "zeroinc");
    ...
    expect((await fs.lstat(path.join(skillsHome, "zeroinc-create-agent"))).isSymbolicLink()).toBe(true);
  });

  it("logs per-skill link failures and continues without throwing", async () => {
    const skillsDir = await makeTempDir("zeroinc-cursor-fail-src-");
    const skillsHome = await makeTempDir("zeroinc-cursor-fail-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);
