 ...
 */
import { pluginManifestV1Schema } from "@zeroinc/shared";
import type { ZeroIncPluginManifestV1 } from "@zeroinc/shared";
import { PLUGIN_API_VERSION } from "@zeroinc/shared";
...
export interface PluginManifestValidator {
   ...
   */
  parseOrThrow(input: unknown): ZeroIncPluginManifestV1;
  ...
  return {
    parse(input: unknown): ManifestParseResult {
      ...
      if (result.success) {
        return {
          success: true,
          manifest: result.data as ZeroIncPluginManifestV1,
        };
      }
