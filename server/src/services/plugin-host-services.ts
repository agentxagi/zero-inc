...
import { pluginLogs, agentTaskSessions as agentTaskSessionsTable } from "@zeroinc/db";
 ...
 *
 * This implementation delegates to the core ZeroInc domain services,
  ...
  return {
    ...
    events: {
      ...
      async subscribe(params: { eventPattern: string; filter?: Record<string, unknown> | null }) {
        const handler = async (event: import("@zeroinc/plugin-sdk").PluginEvent) => {
          if (notifyWorker) {
