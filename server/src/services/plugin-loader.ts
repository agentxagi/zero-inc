 ...
 *    (`~/.zeroinc/plugins/`) and `node_modules` for packages matching
 *    the `zeroinc-plugin-*` naming convention. Aggregates results with
...
/**
 * Naming convention for npm-published ZeroInc plugins.
 * Packages matching this pattern are considered ZeroInc plugins.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export const NPM_PLUGIN_PACKAGE_PREFIX = "zeroinc-plugin-";
...
export interface DiscoveredPlugin {
  ...
  manifest: ZeroIncPluginManifestV1 | null;
}
...
export type PluginSource =
  | "local-filesystem"  // ~/.zeroinc/plugins/ local directory
  | "npm"               // npm packages matching zeroinc-plugin-* convention
...
function getDeclaredPageRoutePaths(manifest: ZeroIncPluginManifestV1): string[] {
...
export interface PluginLoaderOptions {
  ...
  /**
   * Whether to discover installed npm packages matching the zeroinc-plugin-*
   * naming convention.
   * Defaults to true.
   */
...
export interface PluginInstallOptions {
  /**
   * npm package name to install (e.g. "zeroinc-plugin-linear" or "@acme/plugin-linear").
...
export interface PluginRuntimeServices {
   ...
   */
  buildHostHandlers: (pluginId: string, manifest: ZeroIncPluginManifestV1) => WorkerToHostHandlers;
  /**
...
export interface PluginLoader {
  ...
  /**
   * Discover ZeroInc plugins installed as npm packages in the current
   * Node.js environment matching the "zeroinc-plugin-*" naming convention.
   *
   ...
   * the "zeroincPlugin.manifest" key, loads the manifest module, and
   * validates it against the plugin manifest schema.
   *
   * Returns null if the package is not a ZeroInc plugin.
   * Throws if the package is a ZeroInc plugin but the manifest is invalid.
   *
   * @see PLUGIN_SPEC.md §10 — Package Contract
   */
  loadManifest(packagePath: string): Promise<ZeroIncPluginManifestV1 | null>;
  ...
  upgradePlugin(pluginId: string, options: Omit<PluginInstallOptions, "installDir">): Promise<{
    oldManifest: ZeroIncPluginManifestV1;
    newManifest: ZeroIncPluginManifestV1;
    discovered: DiscoveredPlugin;
  }>;
...
/**
 * Check whether a package name matches the ZeroInc plugin naming convention.
 * Accepts both the "zeroinc-plugin-" prefix and scoped "@scope/plugin-" packages.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export function isPluginPackageName(name: string): boolean {
  if (name.startsWith(NPM_PLUGIN_PACKAGE_PREFIX)) return true;
  // Also accept scoped packages like @acme/plugin-linear or @zeroinc/plugin-*
  if (name.includes("/")) {
 ...
 *
 * The spec defines a "zeroincPlugin" key in package.json with a "manifest"
...
function resolveManifestPath(
  ...
  const zeroincPlugin = pkgJson["zeroincPlugin"];
  if (
    zeroincPlugin !== null &&
    typeof zeroincPlugin === "object" &&
    !Array.isArray(zeroincPlugin)
  ) {
    const manifestRelPath = (zeroincPlugin as Record<string, unknown>)[
      "manifest"
    ];
...
function getMinimumHostVersion(manifest: ZeroIncPluginManifestV1): string | undefined {
  return manifest.minimumHostVersion ?? manifest.minimumZeroIncVersion;
}
 ...
 *   packageName: "zeroinc-plugin-linear",
 *   version: "^1.0.0",
 * });
 * ```
 *
...
export function pluginLoader(
  ...
  async function assertPageRoutePathsAvailable(manifest: ZeroIncPluginManifestV1): Promise<void> {
    ...
    for (const plugin of installedPlugins) {
      ...
      const installedManifest = plugin.manifestJson as ZeroIncPluginManifestV1 | null;
  ...
  async function fetchAndValidate(
    ...
    if (!manifestPath || !existsSync(manifestPath)) {
      throw new Error(
        `Package ${resolvedPackageName} at ${resolvedPackagePath} does not appear to be a ZeroInc plugin (no manifest found).`,
      );
    }
  ...
  async function loadManifestFromPath(
    manifestPath: string,
  ): Promise<ZeroIncPluginManifestV1> {
    let raw: unknown;
   ...
   * if the package is not a ZeroInc plugin.
   */
  async function buildDiscoveredPlugin(
    ...
    const hasZeroIncPlugin = "zeroincPlugin" in pkgJson;
    const nameMatchesConvention = isPluginPackageName(packageName);

    if (!hasZeroIncPlugin && !nameMatchesConvention) {
      return null;
    }
  ...
  return {
    ...
    async loadManifest(packagePath: string): Promise<ZeroIncPluginManifestV1 | null> {
      const pkgJson = await readPackageJson(packagePath);
      if (!pkgJson) return null;
    ...
    async upgradePlugin(
      ...
      const plugin = (await registry.getById(pluginId)) as {
        ...
        manifestJson: ZeroIncPluginManifestV1;
      } | null;
  ...
  async function activatePlugin(plugin: PluginRecord): Promise<PluginLoadResult> {
    ...
    try {
      ...
      // (for example @zeroinc/shared exports). Run those workers through
