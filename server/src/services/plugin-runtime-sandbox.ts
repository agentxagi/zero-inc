...
import type { ZeroIncPluginManifestV1 } from "@zeroinc/shared";
...
export async function loadPluginModuleInSandbox(
  ...
  const loadModuleSync = (modulePath: string): Record<string, unknown> => {
    ...
    const sandboxArgs = {
      __zeroinc_exports: module.exports,
      __zeroinc_module: module,
      __zeroinc_require: requireInSandbox,
      __zeroinc_filename: realPath,
      __zeroinc_dirname: path.dirname(realPath),
    };
    // Temporarily inject args into the context, run, then remove to avoid pollution.
    Object.assign(context, sandboxArgs);
    const wrapped = `(function (exports, module, require, __filename, __dirname) {\n${code}\n})(__zeroinc_exports, __zeroinc_module, __zeroinc_require, __zeroinc_filename, __zeroinc_dirname)`;
