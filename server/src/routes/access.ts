...
import type { DeploymentExposure, DeploymentMode } from "@zeroinc/shared";
import {
  forbidden,
  conflict,
  notFound,
...
function readSkillMarkdown(skillName: string): string | null {
  ...
  if (
    normalized !== "zeroinc" &&
    normalized !== "zeroinc-create-agent" &&
    normalized !== "zeroinc-create-plugin" &&
...
/** Resolve the ZeroInc repo skills directory (built-in / managed skills). */
function resolveZeroIncSkillsDir(): string | null {
  const moduleDir = path.dirname(fileURLToPath(import.meta.url));
  const candidates = [
...
function listAvailableSkills(): AvailableSkill[] {
  ...
  const zeroincSkillsDir = resolveZeroIncSkillsDir();

  // Build set of ZeroInc-managed skill names
  const zeroincSkillNames = new Set<string>();
  if (zeroincSkillsDir) {
    try {
      for (const entry of fs.readdirSync(zeroincSkillsDir, { withFileTypes: true })) {
        if (entry.isDirectory()) zeroincSkillNames.add(entry.name);
      }
    } catch { /* skip */ }
  }
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      skills.push({
        name: entry.name,
        description,
        isZeroIncManaged: zeroincSkillNames.has(entry.name),
      });
    }
...
export function buildJoinDefaultsPayloadForAccept(input: {
  ...
  if (!nonEmptyTrimmedString(merged.zeroincApiUrl)) {
    const legacyZeroIncApiUrl = nonEmptyTrimmedString(input.zeroincApiUrl);
    if (legacyZeroIncApiUrl) merged.zeroincApiUrl = legacyZeroIncApiUrl;
  }
...
function summarizeOpenClawGatewayDefaultsForLog(defaultsPayload: unknown) {
  ...
  return {
    ...
    zeroincApiUrl: defaults
      ? nonEmptyTrimmedString(defaults.zeroincApiUrl)
      : null,
...
export function normalizeAgentDefaultsForJoin(input: {
  ...
  const rawZeroIncApiUrl =
    typeof defaults.zeroincApiUrl === "string"
      ? defaults.zeroincApiUrl.trim()
      : "";
  if (rawZeroIncApiUrl) {
    try {
      const parsedZeroIncApiUrl = new URL(rawZeroIncApiUrl);
      if (
        parsedZeroIncApiUrl.protocol !== "http:" &&
        parsedZeroIncApiUrl.protocol !== "https:"
      ) {
        diagnostics.push({
          code: "openclaw_gateway_zeroinc_api_url_protocol",
          level: "warn",
          message: `zeroincApiUrl must use http:// or https:// (got ${parsedZeroIncApiUrl.protocol}).`
        });
      } else {
        normalized.zeroincApiUrl = parsedZeroIncApiUrl.toString();
        diagnostics.push({
          code: "openclaw_gateway_zeroinc_api_url_configured",
          level: "info",
          message: `zeroincApiUrl set to ${parsedZeroIncApiUrl.toString()}`
        });
      }
    } catch {
      diagnostics.push({
        code: "openclaw_gateway_zeroinc_api_url_invalid",
        level: "warn",
        message: `Invalid zeroincApiUrl: ${rawZeroIncApiUrl}`
      });
    }
  }
...
function buildOnboardingDiscoveryDiagnostics(input: {
  ...
  if (apiHost && isLoopbackHost(apiHost)) {
    diagnostics.push({
      ...
      level: "warn",
      message:
        "Onboarding URL resolves to loopback hostname. Remote OpenClaw agents cannot reach localhost on your ZeroInc host.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      level: "warn",
      message: "ZeroInc is bound to loopback in authenticated/private mode.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      hint: `Run pnpm zeroinc allowed-hostname ${apiHost}`
    });
  }
...
function buildInviteOnboardingManifest(
  ...
  const baseUrl = requestBaseUrl(req);
  const skillPath = "/api/skills/zeroinc";
  ...
  return {
    ...
    onboarding: {
      instructions:
        "Join as an OpenClaw Gateway agent, save your one-time claim secret, wait for board approval, then claim your API key. Save the claim response token to ~/.openclaw/workspace/zeroinc-claimed-api-key.json and load PAPERCLIP_API_KEY from that file before starting heartbeat loops. You MUST submit adapterType='openclaw_gateway', set agentDefaultsPayload.url to your ws:// or wss:// OpenClaw gateway endpoint, and include agentDefaultsPayload.headers.x-openclaw-token (or legacy x-openclaw-auth).",
      ...
      requiredFields: {
        ...
        agentDefaultsPayload:
          "Adapter config for OpenClaw gateway. MUST include url (ws:// or wss://) and headers.x-openclaw-token (or legacy x-openclaw-auth). Optional fields: zeroincApiUrl, waitTimeoutMs, sessionKeyStrategy, sessionKey, role, scopes, disableDeviceAuth, devicePrivateKeyPem."
      },
      ...
      connectivity: {
        ...
        guidance:
          ...
          opts.deploymentExposure === "private"
            ? "If OpenClaw runs on another machine, ensure the ZeroInc hostname is reachable and allowed via `pnpm zeroinc allowed-hostname <host>`."
            : "Ensure OpenClaw can reach this ZeroInc API base URL for invite, claim, and skill bootstrap calls."
      },
      textInstructions: {
      ...
      skill: {
        ...
        path: skillPath,
        url: skillUrl,
        installPath: "~/.openclaw/skills/zeroinc/SKILL.md"
      }
    }
  };
}
...
export function buildInviteOnboardingTextDocument(
  ...
  appendBlock(`
    # ZeroInc OpenClaw Gateway Onboarding
  ...
  appendBlock(`
    ...
    BODY="$(node -e '
      ...
      const body = {
        ...
        agentDefaultsPayload: {
          ...
          zeroincApiUrl: "http://host.docker.internal:3100",
    ...
    - Keep device auth enabled (recommended). If devicePrivateKeyPem is omitted, ZeroInc generates and persists one during join so pairing approvals are stable.
    ...
    Body (JSON):
    {
      ...
      "agentDefaultsPayload": {
        ...
        "zeroincApiUrl": "https://zeroinc-hostname-your-agent-can-reach:3100",
    ...
    The board approves the join request in ZeroInc before key claim is allowed.
    ...
    - ~/.openclaw/workspace/zeroinc-claimed-api-key.json
    chmod 600 ~/.openclaw/workspace/zeroinc-claimed-api-key.json
    ...
    ## Step 4: Install ZeroInc skill in OpenClaw
    ...
    ${
      ...
      "Ensure ZeroInc is reachable from your OpenClaw runtime."
    }
  `);
  ...
  if (connectionCandidates.length > 0) {
    lines.push("## Suggested ZeroInc base URLs to try");
    ...
    appendBlock(`
      ...
      - set the first reachable candidate as agentDefaultsPayload.zeroincApiUrl when submitting your join request

      If none are reachable: ask your human operator for a reachable hostname/address and help them update network configuration.
      For authenticated/private mode, they may need:
      - pnpm zeroinc allowed-hostname <host>
      - then restart ZeroInc and retry onboarding.
    `);
  }
...
async function resolveActorEmail(db: Db, req: Request): Promise<string | null> {
  if (isLocalImplicit(req)) return "local@zeroinc.local";
...
export function accessRoutes(
  ...
  router.get("/skills/index", (_req, res) => {
    res.json({
      skills: [
        { name: "zeroinc", path: "/api/skills/zeroinc" },
        {
          name: "para-memory-files",
          path: "/api/skills/para-memory-files"
        },
        {
          name: "zeroinc-create-agent",
          path: "/api/skills/zeroinc-create-agent"
        }
      ]
    });
  });
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      const gatewayDefaultsPayload =
        requestType === "agent"
          ? buildJoinDefaultsPayloadForAccept({
              ...
              zeroincApiUrl: req.body.zeroincApiUrl ?? null,
      ...
      if (requestType === "agent" && adapterType === "openclaw_gateway") {
        ...
        if (
          expectedDefaults.zeroincApiUrl &&
          !persistedDefaults.zeroincApiUrl
        ) {
          missingPersistedFields.push("zeroincApiUrl");
        }
