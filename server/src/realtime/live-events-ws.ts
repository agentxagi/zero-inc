...
import { agentApiKeys, companyMemberships, instanceUserRoles } from "@zeroinc/db";
import type { DeploymentMode } from "@zeroinc/shared";
...
interface IncomingMessageWithContext extends IncomingMessage {
  zeroincUpgradeContext?: UpgradeContext;
}
...
export function setupLiveEventsWebSocketServer(
  ...
  wss.on("connection", (socket: WsSocket, req: IncomingMessage) => {
    const context = (req as IncomingMessageWithContext).zeroincUpgradeContext;
    if (!context) {
  ...
  server.on("upgrade", (req, socket, head) => {
    ...
    void authorizeUpgrade(db, req, companyId, url, {
      ...
      .then((context) => {
        ...
        reqWithContext.zeroincUpgradeContext = context;
