...
How to get OpenClaw running in a Docker container for local development and testing the ZeroInc OpenClaw adapter integration.

## Automated Join Smoke Test (Recommended First)

ZeroInc includes an end-to-end join smoke harness:
...
- writes isolated smoke config under `~/.openclaw-zeroinc-smoke/openclaw.json` and Docker `.env`
...
- probes and prints a ZeroInc host URL that is reachable from inside OpenClaw Docker
...
- `OPENCLAW_CONFIG_DIR` (default `~/.openclaw-zeroinc-smoke`)
...
If your ZeroInc deployment is `authenticated`, provide auth context:
...
- Inside OpenClaw Docker, `127.0.0.1` points to the container itself, not your host ZeroInc server.
- For invite/onboarding URLs consumed by OpenClaw in Docker, use the script-printed ZeroInc URL (typically `http://host.docker.internal:3100`).
- If ZeroInc rejects the container-visible host with a hostname error, allow it from host:

```bash
pnpm zeroinc allowed-hostname host.docker.internal
```

Then restart ZeroInc and rerun the smoke script.
