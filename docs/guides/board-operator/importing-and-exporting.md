...
```sh
zeroinc company export <company-id> --out ./my-export
```
...
zeroinc company export abc123 --out ./backup --include company,agents,projects

# Export everything including tasks and skills
zeroinc company export abc123 --out ./full-export --include company,agents,projects,tasks,skills

# Export only specific skills
zeroinc company export abc123 --out ./skills-only --include skills --skills review,deploy
```
...
```sh
# From a local folder
zeroinc company import ./my-export

# From a GitHub URL
zeroinc company import https://github.com/org/repo

# From a GitHub subfolder
zeroinc company import https://github.com/org/repo/tree/main/companies/acme

# From GitHub shorthand
zeroinc company import org/repo
zeroinc company import org/repo/companies/acme
```
...
```sh
zeroinc company import org/repo --target existing --company-id abc123 --dry-run
```
...
```sh
zeroinc company import org/company-templates/engineering-team \
  --target new \
...
```sh
zeroinc company import ./shared-agents \
  --target existing \
  --company-id abc123 \
  --include agents \
  --collision rename
...
```sh
zeroinc company import org/repo --ref v2.0.0 --dry-run
```

**Non-interactive import (CI/scripts):**

```sh
zeroinc company import ./package \
  --target new \
  --yes \
  --json
```
