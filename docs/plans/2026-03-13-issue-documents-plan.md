...
Add first-class **documents** to ZeroInc as editable, revisioned, company-scoped text artifacts that can be linked to issues.
...
The `plan` key is conventional and reserved by ZeroInc workflow/docs.
...
Update the ZeroInc agent workflow so planning no longer edits the issue description.

Required changes:

- update `skills/zeroinc/SKILL.md`
