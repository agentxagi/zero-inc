...
- Running a Python script that calls the ZeroInc API
...
1. ZeroInc spawns the configured command as a child process
2. Standard ZeroInc environment variables are injected (`PAPERCLIP_AGENT_ID`, `PAPERCLIP_API_KEY`, etc.)
...
The script can use the injected environment variables to authenticate with the ZeroInc API and perform work.
