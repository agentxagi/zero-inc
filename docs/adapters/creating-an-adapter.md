...
Build a custom adapter to connect ZeroInc to any agent runtime.
...
2. Build environment with `buildZeroIncEnv(agent)` plus context vars
...
`format-event.ts` — pretty-prints stdout for `zeroinc run --watch` using `picocolors`.
...
Make ZeroInc skills discoverable to your agent runtime without writing to the agent's working directory:
