...
ZeroInc supports two runtime modes with different security profiles.
...
```sh
pnpm zeroinc allowed-hostname my-machine
```
...
When migrating from `local_trusted` to `authenticated`, ZeroInc emits a one-time claim URL at startup:
...
```sh
PAPERCLIP_DEPLOYMENT_MODE=authenticated pnpm zeroinc run
```
