...
**ClipHub** sells **entire team configurations** — org charts, agent roles, inter-agent workflows, governance rules, and project templates — for ZeroInc-managed companies.

| Dimension | ClipHub |
|---|---|
| Unit of sale | Team blueprint (multi-agent org) |
| Buyer | Founder / team lead spinning up an AI company |
| Install target | ZeroInc company (agents, projects, governance) |
...
A complete ZeroInc company configuration:
...
Single-agent configurations designed to plug into a ZeroInc org:
...
Portable skill files that any ZeroInc agent can use:

- Markdown skill files with instructions
- Tool configurations and shell scripts
- Compatible with ZeroInc's skill loading system
...
interface Listing {
  ...
  zeroincVersionMin: string;     // Minimum ZeroInc version
...
interface Purchase {
  ...
  listingId: string;
  buyerUserId: string;
  buyerCompanyId: string | null;    // Target ZeroInc company
...
| `POST` | `/api/listings/:id/install` | Install to ZeroInc company |
...
Homepage → Browse marketplace → Filter by type/category
  ...
  → Select target ZeroInc company (or create new)
  → ClipHub API calls ZeroInc API to:
  ...
  → Redirect to ZeroInc dashboard with new team running
```
...
### 5.3 Creator: Export from ZeroInc → Publish

```
Running ZeroInc company → "Export as Blueprint" (CLI or UI)
  → ZeroInc exports:
...
- **Color palette**: Dark ink primary, warm sand backgrounds, accent color for CTAs (ZeroInc brand blue/purple)
...
| Install flow | One-click deploy to ZeroInc company |
...
{
  "targetCompanyId": "uuid",         // Existing ZeroInc company
...
3. For each agent in blueprint:
   - `POST /api/companies/:id/agents` (if `zeroinc-create-agent` supports it, or via approval flow)
...
- **Frontend**: Next.js (React), Tailwind CSS, same UI framework as ZeroInc
- **Backend**: Node.js API (or extend ZeroInc server)
- **Database**: Postgres (can share ZeroInc's DB or separate)
- **Payments**: Stripe Connect (marketplace mode)
- **Storage**: S3/R2 for listing bundles and images
- **Auth**: Shared with ZeroInc auth (or OAuth2)

### 9.2 Integration with ZeroInc

ClipHub can be:
- **Option A**: A separate app that calls ZeroInc's API to install blueprints
- **Option B**: A built-in section of the ZeroInc UI (`/marketplace` route)

Option B is simpler for MVP — adds routes to the existing ZeroInc UI and API.
...
- [ ] Install flow: blueprint → ZeroInc company
...
- [ ] "Export from ZeroInc" CLI command
