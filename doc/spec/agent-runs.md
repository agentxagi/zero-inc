...
This spec defines how ZeroInc actually runs agents while staying runtime-agnostic.
...
1. ZeroInc is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, ZeroInc manages lifecycle, and on exit ZeroInc parses JSON output and updates state.
...
- `run.startedAt`
- `heartbeat.reason`
- `zeroinc.skill` (shared ZeroInc skill text block)
