...
ClipHub is the public registry where people share, discover, and download ZeroInc company configurations. A company template is a portable artifact containing an entire org — agents, reporting structure, adapter configs, role definitions, seed tasks — ready to spin up with one command.

---

## What It Is

ClipHub is to ZeroInc what a package registry is to a programming language. ZeroInc already supports exportable org configs (see [SPEC.md](./SPEC.md) §2). ClipHub is the public directory where those exports live.

A user builds a working company in ZeroInc — a dev shop, a marketing agency, a research lab, a content studio — exports the template, and publishes it to ClipHub. Anyone can browse, search, download, and spin up that company on their own ZeroInc instance.
...
A ClipHub package is a **company template export** — the portable artifact format defined in the ZeroInc spec. It contains:
...
**Install (fresh start):**
```
zeroinc install cliphub:<publisher>/<company-slug>
```
Downloads the template and creates a new company in your local ZeroInc instance. You add your own API keys, set budgets, customize agents, and hit go.
...
From within ZeroInc, export your company as a template, then publish:

```
zeroinc export --template my-company
zeroinc publish cliphub my-company
```
...
ClipHub is a **separate service** from ZeroInc itself. ZeroInc is self-hosted; ClipHub is a hosted registry that ZeroInc instances talk to.
...
| **ZeroInc CLI** | `zeroinc install`, `zeroinc publish`, `zeroinc cliphub sync` — built into ZeroInc |
| **ZeroInc UI** | "Browse ClipHub" panel in the ZeroInc web UI for discovering templates without leaving the app |

### Tech Stack

| Layer | Technology |
|---|---|
| Frontend | React + Vite (consistent with ZeroInc) |
| Backend | TypeScript + Hono (consistent with ZeroInc) |
| Database | PostgreSQL |
...
4. Run `zeroinc install cliphub:acme/lean-saas-shop`
5. ZeroInc creates the company locally with all agents pre-configured
6. Set your API keys, adjust budgets, add your initial tasks
7. Hit go

### "I built something great and want to share it"

1. Build and iterate on a company in ZeroInc until it works well
2. Export: `zeroinc export --template my-agency`
3. Publish: `zeroinc publish cliphub my-agency`
...
3. Install just that agent: `zeroinc install cliphub:acme/senior-python-eng --agent`
4. Assign it to a manager in your existing company
5. Done

---

## Relationship to ZeroInc

ClipHub is **not required** to use ZeroInc. You can build companies entirely from scratch without ever touching ClipHub. But ClipHub dramatically lowers the barrier to entry:
...
ClipHub is to ZeroInc what a package registry is to a language runtime: optional, but transformative.
...
- [ ] `zeroinc install cliphub:<publisher>/<slug>` CLI command
...
- [ ] "Browse ClipHub" panel in ZeroInc web UI
- [ ] `zeroinc cliphub sync` for bulk publishing
...
- Running companies on ClipHub (it's a registry, not a runtime — consistent with ZeroInc's own philosophy)
