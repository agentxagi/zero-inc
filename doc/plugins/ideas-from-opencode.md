...
ZeroInc V1 explicitly excludes a plugin framework in [doc/SPEC-implementation.md](../SPEC-implementation.md), but the long-horizon spec says the architecture should leave room for extensions. This report studies the `opencode` plugin system and translates the useful patterns into a ZeroInc-shaped design.

Assumption for this document: ZeroInc is a single-tenant operator-controlled instance. Plugin installation should therefore be global across the instance. "Companies" are still first-class ZeroInc objects, but they are organizational records, not tenant-isolation boundaries for plugin trust or installation.
...
That model works well for a local coding tool. It should not be copied literally into ZeroInc.

The main conclusion is:

- ZeroInc should copy `opencode`'s typed SDK, deterministic loading, low authoring friction, and clear extension surfaces.
- ZeroInc should not copy `opencode`'s trust model, project-local plugin loading, "override by name collision" behavior, or arbitrary in-process mutation hooks for core business logic.
- ZeroInc should use multiple extension classes instead of one generic plugin bag:
...
If ZeroInc does this well, the examples you listed become straightforward:
...
Relevant ZeroInc files reviewed for current extension seams:
...
It is too dangerous for ZeroInc core actions.

However, the concept of plugins contributing agent-usable tools is very valuable for ZeroInc — as long as plugin tools are namespaced (cannot shadow core tools) and capability-gated.
...
ZeroInc should absolutely do this.
...
ZeroInc should copy this discipline.
...
ZeroInc should adopt this style for plugin actions, automations, and UI schemas.
...
ZeroInc can benefit from that with adapters, secret backends, storage providers, and connector modules.
...
That is the correct mindset for ZeroInc too.

## What ZeroInc Should Not Copy Directly
...
ZeroInc is a control plane for an operator-managed instance with company objects.
...
ZeroInc is not project-scoped. It is instance-scoped.
The comparable unit is:

- instance-installed plugin package

ZeroInc should not auto-load arbitrary code from a workspace repo like `.zeroinc/plugins` or project directories.
...
For ZeroInc, equivalent hooks into:
...
ZeroInc should not allow plugins to silently replace:
...
For ZeroInc it would be risky because it combines:
...
ZeroInc should require an explicit operator install step.

## Why ZeroInc Needs A Different Shape

The products are solving different problems.

| Topic | OpenCode | ZeroInc |
|---|---|---|
| Primary unit | local project/worktree | single-tenant operator instance with company objects |
| Trust assumption | local power user on own machine | operator managing one trusted ZeroInc instance |
...
That means ZeroInc should borrow the good ideas from `opencode` but use a stricter architecture.

## ZeroInc Already Has Useful Pre-Plugin Seams

ZeroInc has several extension-like seams already:
...
This is good news.
ZeroInc does not need to invent extensibility from scratch.
It needs to unify and harden existing seams.

## Recommended ZeroInc Plugin Model
...
ZeroInc already has this pattern implicitly:
...
- `plugin` means integration code that talks to ZeroInc through a typed plugin protocol and capability model
...
- invoke explicit ZeroInc actions through the API
...
**What the host controls:** where plugin components appear, the bridge API, capability enforcement, and shared UI primitives (`@zeroinc/plugin-sdk/ui`) with design tokens and common components.
...
ZeroInc should treat plugin installation as a global instance-level action.

Examples:

- install `@zeroinc/plugin-linear` once
- make it available everywhere immediately
- optionally store mappings over ZeroInc objects if one company maps to a different Linear team than another

## 6. Use project workspaces as the primary anchor for local tooling

ZeroInc already has a concrete workspace model for projects:
...
`opencode` makes tools a first-class extension point. This is one of the highest-value surfaces for ZeroInc too.
...
Plugins should be able to emit custom events that other plugins can subscribe to. For example, the git plugin detects a push and emits `plugin.@zeroinc/plugin-git.push-detected`. The GitHub Issues plugin subscribes to that event and updates PR links.
...
A `@zeroinc/plugin-test-harness` package should provide a mock host with in-memory stores, synthetic event emission, and `getData`/`performAction`/`executeTool` simulation. Plugin authors should be able to write unit tests without a running ZeroInc instance.

A `create-zeroinc-plugin` CLI should scaffold a working plugin with manifest, worker, UI bundle, test file, and build config.

Low authoring friction was called out as one of `opencode`'s best qualities. The test harness and starter template are how ZeroInc achieves the same.

## 14. Support hot plugin lifecycle

Plugin install, uninstall, upgrade, and config changes should take effect without restarting the ZeroInc server. This is critical for developer workflow and operator experience.
...
`opencode` does not have a formal SDK versioning story because plugins run in-process and are effectively pinned to the current runtime. ZeroInc's out-of-process model means plugins may be built against one SDK version and run on a host that has moved forward. This needs explicit rules.

Recommended approach:

- **Single SDK package**: `@zeroinc/plugin-sdk` with subpath exports — root for worker code, `/ui` for frontend code. One dependency, one version, one changelog.
- **SDK major version = API version**: `@zeroinc/plugin-sdk@2.x` targets `apiVersion: 2`. Plugins built with SDK 1.x declare `apiVersion: 1` and continue to work.
...
## A Concrete SDK Shape For ZeroInc

An intentionally narrow first pass could look like this:

```ts
import { definePlugin, z } from "@zeroinc/plugin-sdk";

export default definePlugin({
  id: "@zeroinc/plugin-linear",
  version: "0.1.0",
  ...
  async register(ctx) {
    ctx.jobs.register("linear-pull", { cron: "*/5 * * * *" }, async (job) => {
      // sync Linear issues into plugin-owned state or explicit ZeroInc entities
    });
    ...
    ctx.events.on("plugin.@zeroinc/plugin-git.push-detected", async (event) => {
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, ErrorBoundary } from "@zeroinc/plugin-sdk/ui";
...
Any ZeroInc plugin system has to preserve core control-plane invariants from the repo docs.
...
If it needs mappings over specific ZeroInc objects, those are plugin data, not enable/disable boundaries.
...
- `actor_type = plugin`
- `actor_id = <plugin-id>` (e.g. `@zeroinc/plugin-linear`)
...
- reuse core tables when the data is clearly part of ZeroInc itself
...
If a concept is becoming part of ZeroInc's actual product model, it should get a normal first-party table.

Examples:

- `project_workspaces` is already a core table because project workspaces are now part of ZeroInc itself
...
### 3. Scope plugin data to ZeroInc objects before adding custom schemas

A lot of plugin data naturally hangs off existing ZeroInc objects:
...
If ZeroInc eventually needs extension-owned tables, I would only allow that for:
...
Package idea: `@zeroinc/plugin-workspace-files`
...
Package idea: `@zeroinc/plugin-terminal`
...
Package idea: `@zeroinc/plugin-git`
...
- Board opens a project page and reviews the diff for that project's workspace without leaving ZeroInc.
- Board reviews the diff after a run without leaving ZeroInc.
...
- optional `events.emit` (e.g. `plugin.@zeroinc/plugin-git.push-detected`)
- `activity.log.write`
...
The git plugin can emit `plugin.@zeroinc/plugin-git.push-detected` events that other plugins (e.g. GitHub Issues) subscribe to for cross-plugin coordination.
...
Package idea: `@zeroinc/plugin-linear`

This plugin syncs ZeroInc work with Linear. It is useful for:

- importing backlog from Linear
- linking ZeroInc issues to Linear issues
...
- Plugin settings:
  ...
  - status mapping between ZeroInc and Linear
...
- Board enables the plugin, maps a Linear team, and imports a backlog into ZeroInc.
- ZeroInc issue status changes push to Linear and Linear comments arrive back through webhooks.
...
Package idea: `@zeroinc/plugin-github-issues`

This plugin syncs ZeroInc issues with GitHub Issues and optionally links PRs. It is useful for:

- importing repo backlogs
- mirroring issue status and comments
- linking PRs to ZeroInc issues
- tracking cross-repo work from inside one company view
- bridging engineering workflow with ZeroInc task governance
...
- Plugin settings:
  ...
  - whether new ZeroInc issues should create GitHub issues automatically
...
- Dashboard widget:
  - open PRs linked to active ZeroInc issues
  - webhook failures
  - sync lag metrics

Core workflows:

- Board imports GitHub Issues for a repo into ZeroInc.
- GitHub webhooks update status/comment state in ZeroInc.
- A PR is linked back to the ZeroInc issue so the board can follow delivery status.
...
- `events.subscribe(issue.updated)`
- `events.subscribe(issue.comment.created)`
- `events.subscribe(plugin.@zeroinc/plugin-git.push-detected)` (cross-plugin coordination)
- `jobs.schedule`
- `webhooks.receive`
- `http.outbound`
- `secrets.read-ref`
...
Package idea: `@zeroinc/plugin-grafana`

This plugin surfaces external metrics and dashboards inside ZeroInc. It is useful for:

- company KPI visibility
- infrastructure/incident monitoring
- showing deploy, traffic, latency, or revenue charts next to work
- creating ZeroInc issues from anomalous metrics
...
- Dashboard widgets:
  ...
  - link out to Grafana and link in to the full ZeroInc plugin page
- Full metrics page:
...
- Board sees service degradation or business KPI movement directly on the ZeroInc dashboard.
- Board clicks into the full metrics page to inspect the relevant Grafana panels.
- Board creates a ZeroInc issue from a threshold breach with a metric snapshot attached.
...
- do not make Grafana alerting logic part of ZeroInc core; keep it as additive signal and issue creation

## Child Process / Server Tracking

Package idea: `@zeroinc/plugin-runtime-processes`
...
Package idea: `@zeroinc/plugin-stripe`

This plugin pulls Stripe revenue and subscription data into ZeroInc. It is useful for:
...
- Failed payment spikes or churn events can generate ZeroInc issues for follow-up.
...
- Stripe data should stay additive to ZeroInc core
...
- plugin UI bundle loading, host bridge, `@zeroinc/plugin-sdk/ui`
...
- `@zeroinc/plugin-test-harness` and `create-zeroinc-plugin` starter template
...
ZeroInc should not implement "an OpenCode-style generic in-process hook system."
ZeroInc should implement "a plugin platform with multiple trust tiers":
...
## Concrete Next Steps I Would Take In ZeroInc
...
8. Ship `@zeroinc/plugin-test-harness` and `create-zeroinc-plugin` starter template.
