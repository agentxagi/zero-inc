...
This guide describes the current, implemented way to create a ZeroInc plugin in this repo.
...
- Plugin UI runs as same-origin JavaScript inside the main ZeroInc app.
...
```bash
pnpm --filter @zeroinc/create-zeroinc-plugin build
node packages/plugins/create-zeroinc-plugin/dist/index.js @yourscope/plugin-name --output ./packages/plugins/examples
```

For a plugin that lives outside the ZeroInc repo:

```bash
pnpm --filter @zeroinc/create-zeroinc-plugin build
node packages/plugins/create-zeroinc-plugin/dist/index.js @yourscope/plugin-name \
...
Inside this monorepo, the scaffold uses `workspace:*` for `@zeroinc/plugin-sdk`.

Outside this monorepo, the scaffold snapshots `@zeroinc/plugin-sdk` from the local ZeroInc checkout into a `.zeroinc-sdk/` tarball so you can build and test a plugin without publishing anything to npm first.
...
For local development, install it into ZeroInc from an absolute local path through the plugin manager or API. The server supports local filesystem installs and watches local-path plugins for file changes so worker restarts happen automatically after rebuilds.
...
- `usePluginToast`
- `useHostContext`
- typed slot props from `@zeroinc/plugin-sdk/ui`
