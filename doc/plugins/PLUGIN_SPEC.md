# ZeroInc Plugin System Specification

Status: proposed complete spec for the post-V1 plugin system

This document is the complete specification for ZeroInc's plugin and extension architecture.
...
- Plugin UI bundles currently run as same-origin JavaScript inside the main ZeroInc app. Treat plugin UI as trusted code, not a sandboxed frontend capability boundary.
- Manifest capabilities currently gate worker-side host RPC calls. They do not prevent plugin UI code from calling ordinary ZeroInc HTTP APIs directly.
...
ZeroInc plugin design is based on the following assumptions:

1. ZeroInc is single-tenant and self-hosted.
2. Plugin installation is global to the instance.
3. "Companies" remain core ZeroInc business objects, but they are not plugin trust boundaries.
4. Board governance, approval gates, budget hard-stops, and core task invariants remain owned by ZeroInc core.
...
2. Let plugins add major capabilities without editing ZeroInc core.
...
4. Depend on project-local plugin folders such as `.zeroinc/plugins`.
...
The single ZeroInc deployment an operator installs and controls.

### 5.2 Company

A first-class ZeroInc business object inside the instance.
...
A trusted in-process extension loaded directly by ZeroInc core.
...
An installable instance-wide extension package loaded through the ZeroInc plugin runtime.
...
ZeroInc has two extension classes.
...
ZeroInc already has a concrete workspace model:
...
Plugins live under the ZeroInc instance directory.

Suggested layout:

- `~/.zeroinc/instances/default/plugins/package.json`
- `~/.zeroinc/instances/default/plugins/node_modules/`
- `~/.zeroinc/instances/default/plugins/.cache/`
- `~/.zeroinc/instances/default/data/plugins/<plugin-id>/`
...
ZeroInc should add CLI commands:

- `pnpm zeroinc plugin list`
- `pnpm zeroinc plugin install <package[@version]>`
- `pnpm zeroinc plugin uninstall <plugin-id>`
- `pnpm zeroinc plugin upgrade <plugin-id> [version]`
- `pnpm zeroinc plugin doctor <plugin-id>`
...
- UI slot IDs are automatically namespaced by plugin ID (e.g. `@zeroinc/plugin-linear:sync-health-widget`), so cross-plugin collisions are structurally impossible
...
```ts
export interface ZeroIncPluginManifestV1 {
  id: string;
  apiVersion: 1;
  version: string;
  displayName: string;
...
Plugins may contribute tools that ZeroInc agents can use during runs.
...
- ZeroInc server starts one worker process per installed plugin
...
Receives one typed ZeroInc domain event.
...
export function definePlugin(definition: PluginDefinition): ZeroIncPlugin;

/** Re-exported from Zod for config schema definitions */
export { z } from "zod";

export interface PluginContext {
  manifest: ZeroIncPluginManifestV1;
  config: {
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, StatusBadge } from "@zeroinc/plugin-sdk/ui";
...
- The host provides **design tokens and shared components** via `@zeroinc/plugin-sdk/ui` so plugins can match the host's visual language without being forced to.
...
### 19.0.1 Plugin UI SDK (`@zeroinc/plugin-sdk/ui`)
...
- Plugin bundles must not import from host internals. They may only import from `@zeroinc/plugin-sdk/ui` and their own dependencies.
...
## 19.6 Shared Components In `@zeroinc/plugin-sdk/ui`
...
The `@zeroinc/plugin-sdk/ui` subpath should also export an `ErrorBoundary` component that plugin authors can use to catch rendering errors without crashing the host page.
...
- secret ref fields: any schema property annotated with `"format": "secret-ref"` renders as a secret picker that resolves through the ZeroInc secret provider system rather than a plain text input
...
1. Core ZeroInc data stays in first-party tables.
2. Most plugin-owned data starts in generic extension tables.
3. Plugin data should scope to existing ZeroInc objects before new tables are introduced.
4. Arbitrary third-party schema migrations are out of scope for the first plugin system.

## 21.2 Core Table Reuse

If data becomes part of the actual ZeroInc product model, it should become a first-party table.

Examples:

- `project_workspaces` is already first-party
- if ZeroInc later decides git state is core product data, it should become a first-party table too
...
2. Secret refs resolve through the existing ZeroInc secret provider system.
...
6. The operator may force-purge immediately via CLI: `pnpm zeroinc plugin purge <plugin-id>`.
...
Plugin install, uninstall, upgrade, and config changes **must** take effect without restarting the ZeroInc server. This is a normative requirement, not optional.
...
### 27.1 `@zeroinc/plugin-test-harness`
...
```ts
import { createTestHarness } from "@zeroinc/plugin-test-harness";
...
For developing a plugin against a running ZeroInc instance:

- The operator installs the plugin from a local path: `pnpm zeroinc plugin install ./path/to/plugin`
...
The host should publish a starter template (`create-zeroinc-plugin`) that scaffolds:

- `package.json` with correct `zeroincPlugin` keys
...
- `@zeroinc/plugin-workspace-files`
- `@zeroinc/plugin-terminal`
- `@zeroinc/plugin-git`
- `@zeroinc/plugin-linear`
- `@zeroinc/plugin-github-issues`
- `@zeroinc/plugin-grafana`
- `@zeroinc/plugin-runtime-processes`
- `@zeroinc/plugin-stripe`
...
- `@zeroinc/plugin-sdk` — the complete plugin SDK

The package uses subpath exports to separate worker and UI concerns:

- `@zeroinc/plugin-sdk` — worker-side SDK (context, events, state, tools, logger, `definePlugin`, `z`)
- `@zeroinc/plugin-sdk/ui` — frontend SDK (bridge hooks, shared components, design tokens)
...
2. **Tied to API version**: Each major SDK version corresponds to exactly one plugin `apiVersion`. When `@zeroinc/plugin-sdk@2.x` ships, it targets `apiVersion: 2`. Plugins built with SDK 1.x continue to declare `apiVersion: 1`.
...
1. Plugin author updates `@zeroinc/plugin-sdk` dependency.
...
- webhooks
- settings page
- plugin UI bundle loading, host bridge, and `@zeroinc/plugin-sdk/ui`
...
- `@zeroinc/plugin-test-harness`
- `create-zeroinc-plugin` starter template
...
ZeroInc should not implement a generic in-process hook bag modeled directly after local coding tools.

ZeroInc should implement:
...
That is the complete target design for the ZeroInc plugin system.
