...
ZeroInc already treats budgets as a core control-plane responsibility:
...
This plan defines the concrete budgeting model ZeroInc should implement next.

## Product Goals

ZeroInc should let operators:
...
For the next implementation pass, ZeroInc should enforce these defaults:
...
ZeroInc should separate subscription-included usage from billed spend:
...
ZeroInc should have two threshold classes:
...
ZeroInc must know that a project is budget-paused and enforce it.
...
ZeroInc must also block execution before new work starts.
