...
Add a single storage subsystem for ZeroInc that supports:
...
- API serves attachment bytes through authenticated ZeroInc endpoints.
...
- Default local path is under instance root: `~/.zeroinc/instances/<instanceId>/data/storage`.
...
- `zeroinc onboard` writes a valid `storage` config block by default.
- `zeroinc configure --section storage` can switch between local and s3 modes.
...
- `zeroinc doctor` reports actionable storage status.
