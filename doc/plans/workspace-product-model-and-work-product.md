...
ZeroInc needs to support two very different but equally valid ways of working:
...
Today, ZeroInc already has the beginnings of this model:
...
- making ZeroInc feel like a code review tool instead of a control plane
...
6. Seamlessly upgrade existing ZeroInc users to the new model without forcing disruptive reconfiguration.
7. Support cloud-hosted ZeroInc deployments where execution happens in remote or adapter-managed environments rather than local workers.

## Non-Goals

- Turning ZeroInc into a full code review product
...
This object must be recorded explicitly so that ZeroInc can:
...
ZeroInc should treat PRs as a type of work product linked back to:
...
ZeroInc already has users and existing project/task data. Any new model must preserve continuity.
...
### 6. Cloud-hosted ZeroInc must be a first-class deployment mode

ZeroInc cannot assume that it is running on the same machine as the code.

In cloud deployments, ZeroInc may:
...
- `Runtime service`: a process or service ZeroInc owns or tracks for a workspace
...
It also lets cloud-hosted ZeroInc deployments point at codebases and remotes without pretending the ZeroInc host has direct filesystem access.
...
Without an explicit `execution workspace` record, ZeroInc has nowhere stable to attach:
...
ZeroInc needs a single place to show:
...
- ZeroInc-owned processes should update health/status automatically
- external providers should at least store link, provider, external id, and latest known state
- cloud agents should be able to create work product records without ZeroInc owning the execution host
...
## 1. ZeroInc host and execution host must be decoupled
...
without the ZeroInc host itself running local git or local preview processes.
...
ZeroInc should not copy its product framing wholesale, but there are several good patterns worth applying.
...
This is a strong pattern for ZeroInc.
...
ZeroInc should keep workspace automation centered on a small set of hooks:
...
ZeroInc should provide the equivalent visibility in the project `Code` surface:
...
ZeroInc should treat execution workspaces as the natural home for:
...
ZeroInc should ensure that execution workspaces and work product have clean structured API surfaces, not just UI-only representation.
...
ZeroInc should continue treating cleanup policy as part of the core workspace model:
...
ZeroInc should not adopt these Worktrunk assumptions as universal product rules:

- every execution workspace is a local git worktree
- the ZeroInc host has direct shell and filesystem access
- every workflow is merge-centric
- every user wants developer-tool-level workspace detail in the main navigation

### Product implication

ZeroInc should borrow Worktrunk's good execution patterns while keeping the broader ZeroInc model:
...
That keeps ZeroInc focused on coordination and visibility instead of splitting outputs across many hidden subsystems.
...
- cloud-hosted ZeroInc deployments remain first-class

That is a better fit for ZeroInc than either extreme:
