...
This document defines the V1 design for a ZeroInc adapter that integrates with
...
4. synthesized stdout events for ZeroInc UI/CLI
...
3. **Agent callback auth to ZeroInc** uses a bootstrap exchange flow (no long-lived ZeroInc key in prompt).
4. **Webhooks are V1**, polling remains fallback.
5. **Skill delivery** is fetch-on-demand from ZeroInc endpoints, not full SKILL.md prompt injection.
...
- `zeroincPublicUrl` (optional override; else `PAPERCLIP_PUBLIC_URL` env)
...
1. `adapterConfig.zeroincPublicUrl`
...
Goal: avoid putting long-lived ZeroInc credentials in prompt text.

Flow:

1. Before launch/follow-up, ZeroInc mints a one-time bootstrap token bound to:
   - `agentId`
   - `companyId`
   - `runId`
...
4. ZeroInc validates bootstrap token and returns a run-scoped bearer JWT.
5. Cursor agent uses returned bearer token for all ZeroInc API calls.
...
1. Prompt includes a compact instruction to fetch skills from ZeroInc.
2. After auth exchange, agent fetches:
   - `GET /api/skills/index`
   - `GET /api/skills/zeroinc`
   - `GET /api/skills/zeroinc-create-agent` when needed
...
- include webhook object when enabled:
  - `url: <zeroincPublicUrl>/api/adapters/cursor-cloud/webhooks`
...
4. Route by Cursor `agentId` to active ZeroInc run context.
...
5. `zeroincPublicUrl` present and reachable shape-valid
...
| Skill delivery | local skill dir injection | authenticated fetch from ZeroInc skill endpoints |
| ZeroInc auth | injected local run JWT env var | bootstrap token exchange -> run JWT |
...
2. Attach image payloads from ZeroInc context.
3. Add richer PR metadata surfacing in ZeroInc UI.
