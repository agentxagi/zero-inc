...
Today release smoke testing for published ZeroInc packages is manual and shell-driven:
...
- runs `zeroinc@${PAPERCLIPAI_VERSION}` inside Docker
- waits for health
...
- email: `smoke-admin@zeroinc.local`
- password: `zeroinc-smoke-password`
...
1. A published `zeroinc@canary` Docker install can be smoke-tested by Playwright in CI.
2. A published `zeroinc@latest` Docker install can be smoke-tested by Playwright in CI.
