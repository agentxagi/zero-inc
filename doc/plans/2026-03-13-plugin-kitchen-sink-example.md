...
Add a new first-party example plugin, `Kitchen Sink (Example)`, that demonstrates every currently implemented ZeroInc plugin API surface in one place.
...
The plugin is successful if a contributor can install it and, without reading the SDK first, discover and exercise the current plugin runtime surface area from inside ZeroInc.
...
- package: `@zeroinc/plugin-kitchen-sink-example`
- plugin id: `zeroinc.kitchen-sink-example` or `zeroinc-kitchen-sink-example`

Recommendation: use `zeroinc-kitchen-sink-example` to match current in-repo example naming style.
...
- subscribe to a core ZeroInc event such as `issue.created`
...
Recommendation: default to a ZeroInc-local endpoint or a stable public echo endpoint to avoid flaky docs.
