# ZeroInc Module System

## Overview

ZeroInc's module system lets you extend the control plane with new capabilities — revenue tracking, observability, notifications, dashboards — without forking core. Modules are self-contained packages that register routes, UI pages, database tables, and lifecycle hooks.
...
|---------|-----------|----------------|
| **Module** | A package that extends ZeroInc's API, UI, and data model | Yes |
...
modules/
  observability/
    zeroinc.module.json     # manifest (required)
    src/
...
- **`id`**: Unique identifier, used as the npm package name suffix (`@zeroinc/mod-observability`)
...
```typescript
import type { ModuleAPI } from "@zeroinc/core";
...
```
1. Scan modules/ directory for zeroinc.module.json manifests
...
```jsonc
// zeroinc.config.json (or env vars)
{
  "modules": {
...
export const modulePages = [
  {
    path: "/observability",
    label: "Observability",
    component: lazy(() => import("@zeroinc/mod-observability/ui")),
  },
];
...
export const dashboardWidgets = [
  {
    ...
    label: "Token Burn Rate",
    placement: "dashboard",
    component: lazy(() => import("@zeroinc/mod-observability/ui").then(m => ({ default: m.TokenBurnRateWidget }))),
  },
];
```
  ...
  "modules": [
    {
      ...
      "repo": "github:zeroinc/mod-observability",
      "version": "0.1.0",
  ...
  "templates": [
    {
      ...
      "url": "https://store.zeroinc.ing/templates/startup-in-a-box.json",
      "tags": ["startup", "team"]
    }
  ]
}
...
```bash
pnpm zeroinc store list                    # browse available modules and templates
pnpm zeroinc store install <module-id>     # install a module
pnpm zeroinc store import <template-id>    # import a company template
pnpm zeroinc store export                  # export current company as template
```
...
| **Multi-tenant** | Separate companies/orgs within one ZeroInc instance | `server:started` |
...
4. **Module config** — `zeroinc.config.json` with per-module config, env var interpolation
...
10. **`@zeroinc/module-sdk`** — TypeScript types for `ModuleAPI`, `HookEvent`, `HookHandler`, manifest schema
