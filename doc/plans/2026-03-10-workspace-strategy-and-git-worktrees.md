...
`PAP-447` asks how ZeroInc should support worktree-driven coding workflows for local coding agents without turning that into a universal product requirement.
...
- the workflow should reuse the same ZeroInc instance and embedded database instead of creating a blank environment
- local agent auth should remain low-friction

At the same time, we do not want to hard-code "every agent uses git worktrees" into ZeroInc:

- some operators use ZeroInc to manage ZeroInc and want worktrees heavily
- other operators will not want worktrees at all
- not every adapter runs in a local git repository
- not every adapter runs on the same machine as ZeroInc
- Claude and Codex expose different built-in affordances, so ZeroInc should not overfit to one tool

## Core Product Decision

ZeroInc should model **execution workspaces**, not **worktrees**.
...
- local adapters may receive a real filesystem cwd produced by ZeroInc
...
- ZeroInc should not assume that every adapter can see or use a host filesystem path directly
...
If ZeroInc makes worktrees agent-first, it will blur:
...
- ZeroInc core resolves and records execution workspace state
...
That gives ZeroInc room to support:
...
- ZeroInc handles workspace runtime services under the hood using project/workspace policy
...
Once ZeroInc is creating isolated issue checkouts, it is implicitly touching a bigger workflow:
...
- only add adapter-specific runtime UI when an adapter truly needs settings that ZeroInc cannot infer
...
- when issue moves to done, should ZeroInc auto-commit?
- should it auto-push?
...
ZeroInc already has the right foundation for a project-first model.
...
For server-spawned local adapters, ZeroInc already injects a short-lived local JWT:
...
- Codex CLI does not present a single first-class portable CLI worktree abstraction that ZeroInc should mirror directly

Implication:

- for `codex_local`, ZeroInc should usually create/select the checkout itself and then launch Codex inside that cwd
...
- but ZeroInc should still treat that as an adapter optimization, not the canonical cross-adapter model
...
Because of that, ZeroInc should separate:
...
ZeroInc should be able to express intentions such as:
...
These are not specific to ZeroInc. They are a common property of working in a dev workspace, whether local or remote.
...
```json
{
  "workspaceStrategy": {
    ...
    "worktreeParentDir": ".zeroinc/instances/default/worktrees/projects/{{project.id}}",
...
Do not model this as a ZeroInc-specific `devServer` flag.
...
ZeroInc should distinguish between:
...
ZeroInc should normalize the reported metadata without requiring every adapter to look like a host-local process.
...
In addition to the local git helper, ZeroInc should define a generic runtime service manager contract.
...
- remote sandbox adapter: create its own isolated workspace from repo/ref/branch intent and report the realized remote workspace metadata back to ZeroInc
...
For runtime services specifically, ZeroInc should eventually track normalized fields such as:
...
- runtime services are expressed generically, not as ZeroInc-only dev-server flags
...
3. Inject resolved execution workspace details into `context.zeroincWorkspace` for local adapters and into a generic execution-workspace intent payload for adapters that need structured remote realization.
...
- same embedded DB / ZeroInc instance can be reused through env/config injection when appropriate
...
- ZeroInc can decide whether to reuse or start a fresh service deterministically
...
3. Test against ZeroInc-on-ZeroInc workflows first.
...
7. Workspace runtime services are modeled generically, not as ZeroInc-specific dev-server toggles.
