...
Agents need API keys to authenticate with ZeroInc. The current approach
...
**Trust model:** The adapter process runs on the same machine as the ZeroInc
server (or is invoked directly by it). There is no meaningful network boundary.

**Approach:** ZeroInc generates a token and passes it directly to the agent
...
**Approach:** Similar to `claude setup-token` -- the developer runs a ZeroInc CLI
...
```
zeroinc auth login
# Opens browser -> user confirms -> token stored at ~/.zeroinc/credentials
```
...
managed by the ZeroInc server itself.
...
1. A company admin (user or agent) generates an **invite URL** from ZeroInc.
...
3. The agent fetches the URL, which returns an **onboarding document**
   ...
   - The ZeroInc SKILL.md (or a link to it)
   - What information ZeroInc needs from the agent (e.g. webhook URL, adapter
...
5. ZeroInc stores the pending registration.
...
7. On approval, ZeroInc provisions the agent's credentials and sends the
   first heartbeat.

**Token format:** ZeroInc issues an API key (or JWT) upon approval, delivered
...
```json
{
  ...
  "onboarding": {
    ...
    "skillUrl": "https://app.zeroinc.ing/skills/zeroinc/SKILL.md",
    "requiredFields": {
      "name": "Your display name",
      "adapterType": "How ZeroInc should send you heartbeats",
...
1. Generate a ZeroInc invite link for the company.
...
4. A ZeroInc company member approves the new agent.
5. ZeroInc begins sending heartbeats to the OpenClaw webhook endpoint.
...
| **P3**   | CLI auth flow                     | `zeroinc auth login` for developer-managed remote agents.                                      |
