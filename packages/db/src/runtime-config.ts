...
export type ResolvedDatabaseTarget =
  | {
      mode: "postgres";
      connectionString: string;
      source: "DATABASE_URL" | "zeroinc-env" | "config.database.connectionString";
      configPath: string;
      envPath: string;
    }
  | {
...
function resolveZeroIncHomeDir(): string {
  const envHome = process.env.PAPERCLIP_HOME?.trim();
  if (envHome) return path.resolve(expandHomePrefix(envHome));
  return path.resolve(os.homedir(), ".zeroinc");
}

function resolveZeroIncInstanceId(): string {
...
function resolveDefaultEmbeddedPostgresDir(): string {
  return path.resolve(resolveZeroIncHomeDir(), "instances", resolveZeroIncInstanceId(), "db");
}
...
function findConfigFileFromAncestors(startDir: string): string | null {
  ...
  while (true) {
    const candidate = path.resolve(currentDir, ".zeroinc", CONFIG_BASENAME);
...
function resolveZeroIncConfigPath(): string {
...
function resolveZeroIncEnvPath(configPath: string): string {
...
export function resolveDatabaseTarget(): ResolvedDatabaseTarget {
  ...
  const envPath = resolveZeroIncEnvPath(configPath);
