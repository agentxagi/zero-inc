...
async function createTempDatabase(): Promise<string> {
  const dataDir = fs.mkdtempSync(path.join(os.tmpdir(), "zeroinc-db-client-"));
  ...
  const adminUrl = `postgres://zeroinc:zeroinc@127.0.0.1:${port}/postgres`;
  await ensurePostgresDatabase(adminUrl, "zeroinc");
