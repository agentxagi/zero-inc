...
function packLocalPackage(packagePath: string, outputDir: string): string {
  ...
  const sdkBundleDir = path.join(outputDir, ".zeroinc-sdk");
...
/**
 * Generate a complete ZeroInc plugin starter project.
 *
...
export function scaffoldPluginProject(options: ScaffoldPluginOptions): string {
  ...
  const description = options.description ?? "A ZeroInc plugin";
  ...
  const packageJson = {
    ...
    scripts: {
      ...
      "dev:ui": "zeroinc-plugin-dev-server --root . --ui-dir dist/ui --port 4177",
    ...
    keywords: ["zeroinc", "plugin", category],
    author,
    license: "MIT",
    ...(packedSharedTarball
      ? {
        pnpm: {
          overrides: {
            "@zeroinc/shared": `file:${toPosixPath(path.relative(outputDir, packedSharedTarball))}`,
          },
        },
      }
      : {}),
    devDependencies: {
      ...(packedSharedTarball
        ...
        : {}),
      "@zeroinc/plugin-sdk": sdkDependency,
...
`,
  ...
  writeFile(
    ...
    `import { usePluginAction, usePluginData, type PluginWidgetProps } from "@zeroinc/plugin-sdk/ui";
...
${sdkDependency.startsWith("file:")
  ? `This scaffold snapshots \`@zeroinc/plugin-sdk\` and \`@zeroinc/shared\` from a local ZeroInc checkout at:\n\n\`${toPosixPath(localSdkPath)}\`\n\nThe packed tarballs live in \`.zeroinc-sdk/\` for local development. Before publishing this plugin, switch those dependencies to published package versions once they are available on npm.\n\n`
  : ""}
...
function runCli() {
  ...
  if (!pluginName) {
    ...
    console.error("Usage: create-zeroinc-plugin <name> [--template default|connector|workspace] [--output <dir>] [--sdk-path <zeroinc-sdk-path>]");
    process.exit(1);
  }
