/**
 * Core types for the ZeroInc plugin worker-side SDK.
 *
 * These types define the stable public API surface that plugin workers import
 * from `@zeroinc/plugin-sdk`.  The host provides a concrete implementation
...
// ---------------------------------------------------------------------------
// Re-exports from @zeroinc/shared (plugin authors import from one place)
// ---------------------------------------------------------------------------
...
export interface ScopeKey {
  /** What kind of ZeroInc object this state is scoped to. */
...
export interface PluginEntityUpsert {
  ...
  externalId?: string;
  /** Human-readable title for display in the ZeroInc UI. */
  title?: string;
...
/**
 * `ctx.events` — subscribe to and emit ZeroInc domain events.
 *
...
export interface PluginEventsClient {
  /**
   * Subscribe to a core ZeroInc domain event or a plugin-namespaced event.
   *
 ...
 * This client resolves the reference through the ZeroInc secret provider
...
export interface PluginSecretsClient {
   ...
   * in the ZeroInc secret provider (e.g. `"MY_API_KEY"`).
   *
 ...
 * ```
 *
 * @see usePluginStream in `@zeroinc/plugin-sdk/ui`
 */
 ...
 * @example
 * ```ts
 * import { definePlugin } from "@zeroinc/plugin-sdk";
 *
