 ...
 * channel.
 *
 * The ZeroInc plugin runtime uses JSON-RPC 2.0 over stdio to communicate
...
export type { PluginLauncherRenderContextSnapshot } from "@zeroinc/shared";
 ...
 * we use strings (UUIDs or monotonic counters) for all ZeroInc messages.
 */
...
/**
 * ZeroInc plugin-specific error codes.
 *
...
export interface InitializeParams {
  ...
  instanceInfo: {
    /** UUID of this ZeroInc instance. */
    instanceId: string;
    /** Semver version of the running ZeroInc host. */
    hostVersion: string;
  };
  /** Host API version. */
  apiVersion: number;
