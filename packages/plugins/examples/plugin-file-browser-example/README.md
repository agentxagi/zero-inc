...
Example ZeroInc plugin that demonstrates:
...
```bash
pnpm --filter @zeroinc/plugin-file-browser-example build
pnpm zeroinc plugin install ./packages/plugins/examples/plugin-file-browser-example
```

To uninstall:

```bash
pnpm zeroinc plugin uninstall zeroinc-file-browser-example --force
```
...
- Optional: use `zeroinc-plugin-dev-server` for UI hot-reload with `devUiUrl` in plugin config.
