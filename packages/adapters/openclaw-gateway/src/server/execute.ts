...
import { asNumber, asString, buildZeroIncEnv, parseObject } from "@zeroinc/adapter-utils/server-utils";
...
const DEFAULT_CLIENT_VERSION = "zeroinc";
...
function resolveSessionKey(input: {
  ...
  const fallback = input.configuredSessionKey ?? "zeroinc";
  if (input.strategy === "run") return `zeroinc:run:${input.runId}`;
  if (input.strategy === "issue" && input.issueId) return `zeroinc:issue:${input.issueId}`;
  return fallback;
}
...
function resolveZeroIncApiUrlOverride(value: unknown): string | null {
...
function buildZeroIncEnvForWake(ctx: AdapterExecutionContext, wakePayload: WakePayload): Record<string, string> {
  const zeroincApiUrlOverride = resolveZeroIncApiUrlOverride(ctx.config.zeroincApiUrl);
  const zeroincEnv: Record<string, string> = {
    ...buildZeroIncEnv(ctx.agent),
    PAPERCLIP_RUN_ID: ctx.runId,
  };

  if (zeroincApiUrlOverride) {
    zeroincEnv.PAPERCLIP_API_URL = zeroincApiUrlOverride;
  }
  if (wakePayload.taskId) zeroincEnv.PAPERCLIP_TASK_ID = wakePayload.taskId;
  if (wakePayload.wakeReason) zeroincEnv.PAPERCLIP_WAKE_REASON = wakePayload.wakeReason;
  if (wakePayload.wakeCommentId) zeroincEnv.PAPERCLIP_WAKE_COMMENT_ID = wakePayload.wakeCommentId;
  if (wakePayload.approvalId) zeroincEnv.PAPERCLIP_APPROVAL_ID = wakePayload.approvalId;
  if (wakePayload.approvalStatus) zeroincEnv.PAPERCLIP_APPROVAL_STATUS = wakePayload.approvalStatus;
  if (wakePayload.issueIds.length > 0) {
    zeroincEnv.PAPERCLIP_LINKED_ISSUE_IDS = wakePayload.issueIds.join(",");
  }

  return zeroincEnv;
}

function buildWakeText(payload: WakePayload, zeroincEnv: Record<string, string>): string {
  const claimedApiKeyPath = "~/.openclaw/workspace/zeroinc-claimed-api-key.json";
  const orderedKeys = [
    "PAPERCLIP_RUN_ID",
    "PAPERCLIP_AGENT_ID",
  ...
  for (const key of orderedKeys) {
    const value = zeroincEnv[key];
    if (!value) continue;
    envLines.push(`${key}=${value}`);
  }

  const issueIdHint = payload.taskId ?? payload.issueId ?? "";
  const apiBaseHint = zeroincEnv.PAPERCLIP_API_URL ?? "<set PAPERCLIP_API_URL>";

  const lines = [
    "ZeroInc wake event for a cloud adapter.",
    "",
    ...
    "- Use X-ZeroInc-Run-Id: $PAPERCLIP_RUN_ID on every mutating API call.",
...
function buildStandardZeroIncPayload(
  ctx: AdapterExecutionContext,
  wakePayload: WakePayload,
  zeroincEnv: Record<string, string>,
  payloadTemplate: Record<string, unknown>,
): Record<string, unknown> {
  const templateZeroInc = parseObject(payloadTemplate.zeroinc);
  const workspace = asRecord(ctx.context.zeroincWorkspace);
  const workspaces = Array.isArray(ctx.context.zeroincWorkspaces)
    ? ctx.context.zeroincWorkspaces.filter((entry): entry is Record<string, unknown> => Boolean(asRecord(entry)))
    : [];
  const configuredWorkspaceRuntime = parseObject(ctx.config.workspaceRuntime);
  const runtimeServiceIntents = Array.isArray(ctx.context.zeroincRuntimeServiceIntents)
    ? ctx.context.zeroincRuntimeServiceIntents.filter(
        (entry): entry is Record<string, unknown> => Boolean(asRecord(entry)),
      )
    : [];

  const standardZeroInc: Record<string, unknown> = {
    runId: ctx.runId,
    ...
    apiUrl: zeroincEnv.PAPERCLIP_API_URL ?? null,
  };

  if (workspace) {
    standardZeroInc.workspace = workspace;
  }
  if (workspaces.length > 0) {
    standardZeroInc.workspaces = workspaces;
  }
  if (runtimeServiceIntents.length > 0 || Object.keys(configuredWorkspaceRuntime).length > 0) {
    standardZeroInc.workspaceRuntime = {
...
class GatewayWsClient {
  ...
  close() {
    if (!this.ws) return;
    this.ws.close(1000, "zeroinc-complete");
    this.ws = null;
  }
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const zeroincEnv = buildZeroIncEnvForWake(ctx, wakePayload);
  const wakeText = buildWakeText(wakePayload, zeroincEnv);
  ...
  const zeroincPayload = buildStandardZeroIncPayload(ctx, wakePayload, zeroincEnv, payloadTemplate);
