...
import { asString, parseObject } from "@zeroinc/adapter-utils/server-utils";
    ...
    try {
      ...
      if (probeResult === "ok") {
        ...
        checks.push({
          ...
          hint: "Verify network reachability and gateway URL from the ZeroInc server host.",
        });
      }
    } catch (err) {
      checks.push({
