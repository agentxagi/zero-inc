...
Use when:
- You want ZeroInc to invoke OpenClaw over the Gateway WebSocket protocol.
...
- Your deployment does not permit outbound WebSocket access from the ZeroInc server.
...
- workspaceRuntime (object, optional): desired runtime service intents; ZeroInc forwards these in a standardized zeroinc.workspaceRuntime block for remote execution environments
- timeoutSec (number, optional): adapter timeout in seconds (default 120)
- waitTimeoutMs (number, optional): agent.wait timeout override (default timeoutSec * 1000)
- autoPairOnFirstConnect (boolean, optional): on first "pairing required", attempt device.pair.list/device.pair.approve via shared auth, then retry once (default true)
- zeroincApiUrl (string, optional): absolute ZeroInc base URL advertised in wake text

Session routing fields:
- sessionKeyStrategy (string, optional): issue (default), fixed, or run
- sessionKey (string, optional): fixed session key when strategy=fixed (default zeroinc)

Standard outbound payload additions:
- zeroinc (object): standardized ZeroInc context added to every gateway agent request
- zeroinc.workspace (object, optional): resolved execution workspace for this run
- zeroinc.workspaces (array, optional): additional workspace hints ZeroInc exposed to the run
- zeroinc.workspaceRuntime (object, optional): normalized runtime service intent config for the workspace
