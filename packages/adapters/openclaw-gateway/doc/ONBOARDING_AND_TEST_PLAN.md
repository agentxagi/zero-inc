...
## Scope
This plan is now **gateway-only**. ZeroInc supports OpenClaw through `openclaw_gateway` only.
...
6. OpenClaw claims API key and installs/uses ZeroInc skill.
...
3. In `/new` OpenClaw session, OpenClaw can still create a ZeroInc task.
...
```bash
pnpm --filter @zeroinc/server test:run -- openclaw-gateway
pnpm --filter @zeroinc/server typecheck
pnpm --filter @zeroinc/ui typecheck
pnpm --filter zeroinc typecheck
```
