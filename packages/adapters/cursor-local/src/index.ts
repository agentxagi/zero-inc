...
Use when:
- You want ZeroInc to run Cursor Agent CLI locally as the agent runtime
...
- ZeroInc auto-injects local skills into "~/.cursor/skills" when missing, so Cursor can discover "$zeroinc" and related skills on local runs.
- ZeroInc auto-adds --yolo unless one of --trust/--yolo/-f is already present in extraArgs.
`;
