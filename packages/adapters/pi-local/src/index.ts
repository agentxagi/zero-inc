...
Use when:
- You want ZeroInc to run Pi (the AI coding agent) locally as the agent runtime
...
- ZeroInc requires an explicit \`model\` value for \`pi_local\` agents.
- Sessions are stored in ~/.pi/zeroincs/ and resumed with --session.
