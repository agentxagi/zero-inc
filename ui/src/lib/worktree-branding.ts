...
export function getWorktreeUiBranding(): WorktreeUiBranding | null {
  if (readMetaContent("zeroinc-worktree-enabled") !== "true") return null;

  const name = readMetaContent("zeroinc-worktree-name");
  const color = normalizeHexColor(readMetaContent("zeroinc-worktree-color"));
  ...
  return {
    ...
    name,
    color,
    textColor: normalizeHexColor(readMetaContent("zeroinc-worktree-text-color")) ?? pickReadableTextColor(color),
  };
}
