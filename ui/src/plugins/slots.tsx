 ...
 *
 * Plugin bundles are built with `external: ["@zeroinc/plugin-sdk/ui", "react", "react-dom"]`,
 ...
 *    host's global bridge registry (`globalThis.__zeroincPluginBridge__`)
...
function getShimBlobUrl(specifier: "react" | "react-dom" | "react-dom/client" | "react/jsx-runtime" | "sdk-ui"): string {
  ...
  switch (specifier) {
    case "react":
      source = `
        const R = globalThis.__zeroincPluginBridge__?.react;
        export default R;
    ...
    case "react-dom/client":
      source = `
        const RD = globalThis.__zeroincPluginBridge__?.reactDom;
        export default RD;
    ...
    case "sdk-ui":
      source = `
        const SDK = globalThis.__zeroincPluginBridge__?.sdkUi ?? {};
 ...
 * - `import { ... } from "@zeroinc/plugin-sdk/ui";`
 *
...
function rewriteBareSpecifiers(source: string): string {
  ...
  const rewrites: Record<string, string> = {
    '"@zeroinc/plugin-sdk/ui"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@zeroinc/plugin-sdk/ui'": `'${getShimBlobUrl("sdk-ui")}'`,
    '"@zeroinc/plugin-sdk/ui/hooks"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@zeroinc/plugin-sdk/ui/hooks'": `'${getShimBlobUrl("sdk-ui")}'`,
...
async function importPluginModule(url: string): Promise<Record<string, unknown>> {
  ...
  if (!globalThis.__zeroincPluginBridge__) {
 ...
 * self-register via `window.zeroincPlugins.registerReactComponent()`.
 ...
 * of `@zeroinc/plugin-sdk/ui`, `react`, and `react-dom` resolve to the
