...
import { parseProjectMentionHref } from "@zeroinc/shared";
...
function MermaidDiagramBlock({ source, darkMode }: { source: string; darkMode: boolean }) {
  ...
  useEffect(() => {
    ...
    loadMermaid()
      .then(async (mermaid) => {
        ...
        const rendered = await mermaid.render(`zeroinc-mermaid-${renderId}`, source);
        if (!active) return;
        setSvg(rendered.svg);
      })
      .catch((err) => {
  ...
  return (
    <div className="zeroinc-mermaid">
      {svg ? (
        <div dangerouslySetInnerHTML={{ __html: svg }} />
      ) : (
        <>
          <p className={cn("zeroinc-mermaid-status", error && "zeroinc-mermaid-status-error")}>
            {error ? `Unable to render Mermaid diagram: ${error}` : "Rendering Mermaid diagram..."}
          </p>
          <pre className="zeroinc-mermaid-source">
...
export function MarkdownBody({ children, className }: MarkdownBodyProps) {
  ...
  return (
    <div
      className={cn(
        "zeroinc-markdown prose prose-sm max-w-none break-words overflow-hidden",
            ...
            if (parsed) {
              ...
              return (
                <a
                  ...
                  className="zeroinc-project-mention-chip"
