...
name: release
description: >
  Coordinate a full ZeroInc release across engineering verification, npm,
...
Run the full ZeroInc maintainer release workflow, not just an npm publish.
...
7. If running through ZeroInc, you have issue context for status updates and follow-up task creation.
...
ZeroInc now uses a commit-driven release model:
...
npm view zeroinc@canary version
```
...
- release summary in ZeroInc issue context
