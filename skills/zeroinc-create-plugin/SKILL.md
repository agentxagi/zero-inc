---
name: zeroinc-create-plugin
description: >
  Create new ZeroInc plugins with the current alpha SDK/runtime. Use when
...
# Create a ZeroInc Plugin

Use this skill when the task is to create, scaffold, or document a ZeroInc plugin.
...
node packages/plugins/create-zeroinc-plugin/dist/index.js <npm-package-name> --output <target-dir>
```

For a plugin that lives outside the ZeroInc repo, pass `--sdk-path` and let the scaffold snapshot the local SDK/shared packages into `.zeroinc-sdk/`:

```bash
pnpm --filter @zeroinc/create-zeroinc-plugin build
node packages/plugins/create-zeroinc-plugin/dist/index.js @acme/plugin-name \
...
- is installed into ZeroInc from an absolute local path during development
