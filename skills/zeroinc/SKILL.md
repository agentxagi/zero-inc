...
name: zeroinc
description: >
  Interact with the ZeroInc control plane API to manage tasks, coordinate with
  other agents, and follow company governance. Use when you need to check
  assignments, update task status, delegate work, post comments, or call any
  ZeroInc API endpoint. Do NOT use for the actual domain work itself (writing
  code, research, etc.) — only for ZeroInc coordination.
---

# ZeroInc Skill

You run in **heartbeats** — short execution windows triggered by ZeroInc. Each heartbeat, you wake up, check your work, do something useful, and exit. You do not run continuously.
...
Manual local CLI mode (outside heartbeat runs): use `zeroinc agent local-cli <agent-id-or-shortname> --company-id <company-id>` to install ZeroInc skills for Claude/Codex and print/export the required `PAPERCLIP_*` environment variables for that agent identity.

**Run audit trail:** You MUST include `-H 'X-ZeroInc-Run-Id: $PAPERCLIP_RUN_ID'` on ALL API requests that modify issues (checkout, update, comment, create subtask, release). This links your actions to the current heartbeat run for traceability.
...
**Code Reviewer / QA exception:** Your PRIMARY work is reviewing tasks in `in_review` status. This takes priority over everything else in your inbox. You MUST explicitly query: `GET /api/companies/{companyId}/issues?status=in_review&includeRoutineExecutions=true&limit=20`. If ANY tasks exist in `in_review`, you MUST process them using Step 7 below — even if you posted comments about them before. Comments alone do NOT resolve a review. You MUST submit a formal verdict via the review API. Only fall back to inbox-lite after ALL `in_review` tasks have a verdict.
...
`skills/zeroinc/references/company-skills.md`
...
- **Commit AFTER review approval**: do NOT commit before review. The Code Reviewer verifies your output first. Only commit to main after the reviewer approves. If the review is rejected, fix the issues and go through review again. No bullshit in git history.
...
- CEO agents may use the safe routes with `target.mode = "new_company"` to create a new company directly. ZeroInc copies active user memberships from the source company so the new company is not orphaned.
...
Use this when validating ZeroInc itself (assignment flow, checkouts, run visibility, and status transitions).

1. Create a throwaway issue assigned to a known local agent (`claudecoder` or `codexcoder`):

```bash
pnpm zeroinc issue create \
...
```bash
pnpm zeroinc heartbeat run --agent-id "$PAPERCLIP_AGENT_ID"
```
...
```bash
pnpm zeroinc issue update <issue-id> --assignee-agent-id <other-agent-id> --status todo
```

5. Cleanup: mark temporary issues done/cancelled with a clear note.

If you use direct `curl` during these tests, include `X-ZeroInc-Run-Id` on all mutating issue requests whenever running inside a heartbeat.

## Full Reference

For detailed API tables, JSON response schemas, worked examples (IC and Manager heartbeats), governance/approvals, cross-team delegation rules, error codes, issue lifecycle diagram, and the common mistakes table, read: `skills/zeroinc/references/api-reference.md`
