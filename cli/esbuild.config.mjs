/**
 * esbuild configuration for building the zeroinc CLI for npm.
 *
 * Bundles all workspace packages (@zeroinc/*) into a single file.
...
// to npm and resolved at runtime (e.g. @zeroinc/server uses dynamic import).
...
for (const p of workspacePaths) {
  ...
  for (const name of Object.keys(pkg.dependencies || {})) {
    if (externalWorkspacePackages.has(name)) {
      externals.add(name);
    } else if (!name.startsWith("@zeroinc/")) {
      externals.add(name);
    }
  }
