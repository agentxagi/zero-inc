...
async function startTempDatabase() {
  ...
  const { applyPendingMigrations, ensurePostgresDatabase } = await import("@zeroinc/db");
...
async function waitForServer(
  ...
  while (Date.now() - startedAt < 30_000) {
    if (child.exitCode !== null) {
      throw new Error(
        `zeroinc run exited before healthcheck succeeded.\nstdout:\n${output.stdout.join("")}\nstderr:\n${output.stderr.join("")}`,
      );
    }
...
describe("zeroinc company import/export e2e", () => {
  let tempRoot = "";
  let configPath = "";
  let exportDir = "";
  let apiBase = "";
