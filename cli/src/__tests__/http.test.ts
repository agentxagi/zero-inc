...
import { ApiConnectionError, ApiRequestError, ZeroIncApiClient } from "../client/http.js";

describe("ZeroIncApiClient", () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });
  ...
  it("adds authorization and run-id headers", async () => {
    ...
    expect(headers["x-zeroinc-run-id"]).toBe("run-abc");
