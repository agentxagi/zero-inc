...
function tempConfigPath(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "zeroinc-jwt-env-"));
...
describe("agent jwt env helpers", () => {
  ...
  it("quotes hash-prefixed env values so dotenv round-trips them", () => {
    ...
    expect(readZeroIncEnvEntries(envPath).PAPERCLIP_WORKTREE_COLOR).toBe("#439edb");
  });
});
