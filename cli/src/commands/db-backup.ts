...
import { formatDatabaseBackupResult, runDatabaseBackup } from "@zeroinc/db";
import {
  expandHomePrefix,
...
function resolveConnectionString(configPath?: string): { value: string; source: string } {
  ...
  return {
    value: `postgres://zeroinc:zeroinc@127.0.0.1:${port}/zeroinc`,
...
export async function dbBackupCommand(opts: DbBackupOptions): Promise<void> {
  ...
  p.intro(pc.bgCyan(pc.black(" zeroinc db:backup ")));

  const configPath = resolveConfigPath(opts.config);
  const config = readConfig(opts.config);
  const connection = resolveConnectionString(opts.config);
  const defaultDir = resolveDefaultBackupDir(resolveZeroIncInstanceId());
  ...
  const filenamePrefix = opts.filenamePrefix?.trim() || "zeroinc";
