...
function renderEnvFile(entries: Record<string, string>) {
  const lines = [
    "# ZeroInc environment variables",
    "# Generated by ZeroInc CLI commands",
...
export function resolveZeroIncEnvFile(configPath?: string): string {
...
export function loadZeroIncEnvFile(configPath?: string): void {
...
export function writeAgentJwtEnv(secret: string, filePath = resolveEnvFilePath()): void {
  mergeZeroIncEnvEntries({ [JWT_SECRET_ENV_KEY]: secret }, filePath);
}

export function readZeroIncEnvEntries(filePath = resolveEnvFilePath()): Record<string, string> {
  if (!fs.existsSync(filePath)) return {};
  return parseEnvFile(fs.readFileSync(filePath, "utf-8"));
}

export function writeZeroIncEnvEntries(entries: Record<string, string>, filePath = resolveEnvFilePath()): void {
...
export function mergeZeroIncEnvEntries(
  entries: Record<string, string>,
  filePath = resolveEnvFilePath(),
): Record<string, string> {
  const current = readZeroIncEnvEntries(filePath);
  const next = {
    ...current,
    ...Object.fromEntries(
  ...
  writeZeroIncEnvEntries(next, filePath);
  return next;
}
